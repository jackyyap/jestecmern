...
const EditorModal = () => {
  ...
  const getManuscriptDetails = async (id) => {
      try {
        const result = await axios.get(`${API_URL}/editormodal/${id}`);
        setErrorMsg('');
        setFile(result);
      } catch (error) {
        if (error.response && error.response.status === 400) {
            setErrorMsg('Error while fetching details. Try again.');
        }
      }
  };
          ...
          <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
              <div class="modal-header">
                  <strong>ManuscriptID</strong>: ManuscriptTitle Lorem Ipsum D'olor Sit Amet
